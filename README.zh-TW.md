...
此命令對所有插件執行 70 個 fixture 檢查（有效 fixture 期望 exit 0，無效 fixture 期望 non-zero），不需要任何安裝。
...
一套多 Agent 開發模式，透過認識論分工將單一 agent 錯誤率從 p 壓縮至 p^4。專為安全關鍵程式碼設計：加密函數、財務計算、資料驗證、安全關鍵邏輯。
...
```
單一 Agent (~1x)  →  /debate (~4-5x)  →  /start (~12-18x)
一般 CRUD             架構決策              安全關鍵程式碼
```

**串接使用**：先用 `/debate` 決定方案（What & Why），再用 `/start` 實作（How）。
...
建立 `SPEC.md` 和 `CONSENSUS.md` 模板。請完整填寫 SPEC.md（特別是邊界條件表）。
...
1. **規格確認** — 所有 agent 審查 SPEC.md 的歧義
2. **獨立實作** — 兩個 implementer 在隔離 worktree 中各自實作
3. **對抗審查** — Critic 找 bug + Adversary 三輪攻擊
4. **整合** — Verifier 合併最佳部分，100% SPEC.md 覆蓋

## 代理人說明

| 代理人 | 角色 | 產出 |
|--------|------|------|
| 實作者 A | 獨立防禦性實作 | `IMPL_A_REPORT.md` |
| 實作者 B | 獨立防禦性實作 | `IMPL_B_REPORT.md` |
| 批評者 | 使用 severity 1-5 分級找出問題 | `CRITIQUE.md` |
| 攻擊者 | 三輪紅隊攻擊（邊界、語意、假設） | `ATTACKS.md` |
| 驗證者 | 比較、合併、逐條驗證 SPEC.md | `VERIFICATION.md` |
...
- 關鍵字：「嚴謹審查」、「深度 audit」、「量化決策」、「security review」、「對抗式 review」、「rigorous review」、「deep audit」
- 程式碼觸及：secrets 處理、auth 邊界、密碼學原語、金流、IaC / infra glue、不可信輸入解析器、LLM context 組裝
- 明確呼叫：「use code-audit-rigor」

日常 PR review 直接用 `/review-branch` 即可——這個 skill 在那種情境下是 overkill。

## 五項核心審查紀律原則

1. **先讀完再評分** — 完整讀過每個 in-scope 檔案，才能形成任何意見
2. **「我真的讀了嗎，還是用猜的？」自我提問** — 在提交任何 finding 前
3. **驗證原始碼而非依賴 diff** — 任何 critical 聲明都要 `grep` + `Read` 完整定義
4. **多 agent 共識 ≠ 驗證** — 三個 agent 讀同一份 diff 會同向誤讀，永遠要自己回去驗證原始碼
5. **漏抓真 bug 比誤判昂貴 2 倍** — 不對稱懲罰，驅動保守的 dismissal 邏輯
...
所有插件皆可直接在任何專案中使用。每個代理人都會適應您專案的結構和慣例。
