...
- **scope-ledger 1.0.0** — a per-worktree scope ledger and four fail-open hooks that keep review-driven work from expanding past the original request. The ledger (`.claude/scope-ledger.local.md`) holds the user's goal verbatim, the branch, a review round counter and In scope / Deferred / Log sections. PreToolUse denies the main thread's first source-file edit once when no ledger exists for the current branch and asks for `/scope-ledger:scope init` (subagent edits pass without consuming that one deny); PostToolUse injects the finding-triage policy after every review-entry skill or review-type subagent (findings are input, not a work order — adopt, defer with reason and destination, hand to the user, or log as noise, *before* editing) and counts a round per allowlisted skill call, with a convergence warning from round 3; Stop blocks once per distinct open-item state; SessionStart replays the ledger after compaction or resume. A git-tracked ledger is repository-controlled content and every hook ignores it — the Codex cross-vendor pass caught that SessionStart would otherwise replay a committed file verbatim into context. Designed from measurements, not intuition: across the author's whole session history the todo tool had been called zero times, one four-day session ran 23 security reviews across 17 PRs and ended with the user asking what the finding IDs meant, and a session that did use the task tool still had the user ask five times what was left — the audit backlog lived outside the task list. No existing skill on skills.sh enforces this with hooks; the closest (`piv-fix-review-findings`) has the same policy in prose, and its four-way taxonomy is adopted here. 158 fixture assertions, green under bash 5 and macOS bash 3.2, nine mutations verified red on a copy. The first draft's pre-release review (five adversarial verifiers, Codex, then the pre-push security review) changed seven things before shipping — tracked-ledger handling, then its symlink / submodule bypass, Agent dispatches not counting as rounds, an allowlist instead of keyword stems, subagent pass-through, a bump lock, frontmatter synthesis — each recorded in the plugin's CHANGELOG with its reason. Design: `docs/scope-ledger-design-2026-09-23.md`.
