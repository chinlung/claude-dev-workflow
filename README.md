...
| [Scope Ledger](#scope-ledger-plugin) | Per-worktree scope ledger + four fail-open hooks that triage review findings and stop review-driven scope creep | `/scope-ledger:scope` + PreToolUse / PostToolUse / Stop / SessionStart hooks |
...
This validates every valid fixture exits 0 and every invalid fixture exits non-zero across all plugins (self-test canaries + static fixtures + generated single-field mutations + schema↔validator consistency, no install required; the run prints its own current check count).
