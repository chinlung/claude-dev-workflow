...
- The Bash gate recognises `>`/`>>`, `tee`, `sed -i`, `perl -pi`/`-i`, `cp`, `mv`, `git apply` and `patch`. A formatter, a scaffolder (`artisan make:…`), `python - <<EOF` writing files, or a `pint` run are not recognised; the first-prompt reminder is the backstop for those sessions.
