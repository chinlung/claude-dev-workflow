...
|---|---|---|
| `scope-gate.sh` | PreToolUse `Edit\|Write\|MultiEdit\|NotebookEdit` | First edit of a **source file** (code, shell, SQL, IaC, CI workflow — not Markdown/JSON) inside a git worktree, when the ledger is missing or bound to a different branch → deny **once**, asking for `/scope-ledger:scope init "<goal>"`. The retry passes. Exempt: `.claude/`, `openspec/`, paths outside the project, non-git directories. |
| `scope-review-triage.sh` | PostToolUse `Skill\|Agent` whose skill / subagent type matches `review\|security\|audit\|codex\|simplify\|critic\|adversar\|verif\|debate` | Increments `review_rounds` in the ledger and injects the triage policy as context: findings are input, not a work order; each goes into the ledger first (adopt → In scope with source; later → Deferred with reason and destination; noise → one Log line); nothing is fixed before it is in the ledger; until the goal's In scope items are all ticked, only findings that belong to *this* change and are must-fix are adopted. From round 3 it adds a convergence warning asking for the remaining / deferred lists before another round. With no ledger it says so and points at `init`. |
