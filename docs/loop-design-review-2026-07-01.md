...
**Tier-3 judgment calls — explicit disposition (2026-07-02, closing the two items that were neither done nor listed above):**
- **#4 — code-audit-rigor optional 2-3-angle fan-out above a diff-size threshold: won't-do (deliberate).** The routine `/review-branch` / `/review-pr` tier is intentionally single-shot; multi-angle fan-out already exists as its own product surface (`/audit-review-fix`'s 9-angle review). Adding a size-triggered fan-out to the routine commands would blur the deliberate routine-vs-rigor tiering (the article's "don't over-engineer a simple task"). **Re-open trigger:** a real case where `/review-branch` on a large diff misses something `/audit-review-fix` would have caught, i.e. evidence the tiering boundary is set wrong — not before.
- **#13 — openspec phase-completion-criteria mechanization: won't-do for now (correctly hard, low value).** `PHASE-IDENTIFICATION.md`'s "substantive content, not placeholder" is a genuine judgment call; the multi-session filesystem nature makes a mechanical check inherently harder than the other three plugins' single-session checks, and a shallow proxy (file non-empty) would manufacture the same false confidence this whole review set out to remove. The existing `openspec validate --strict` + the lenient `.cjs` pre-check already gate artifact *structure*. **Re-open trigger:** a defined, falsifiable notion of "phase actually complete" that reads an environmental fact (not an agent assertion) — absent that, per this repo's gate meta-rule, no gate should be added.
...
- **Deeper finding — the `p→p⁴` claim is overstated** (the critic's second blind spot, source-verified, not yet acted on): `implementer-a.md` and `implementer-b.md` are byte-identical except the A/B label — same base model, same SPEC/CONSENSUS, same framing, differing only in worktree output-isolation. Two identically-prompted instances of one model make *correlated* errors on systematic misreadings (the dangerous class); worktree isolation stops plagiarism, not correlated independent error. Combined with the shared-base-model correlation floor (a blind spot in the weights is shared by all 4–6 roles) and `CONSENSUS.md` as a common-mode single point of failure (a wrong Phase-1 resolution is trusted by every downstream channel), the multiplicative `p⁴` does not hold as stated. The real compression is role-diversity-driven (builder vs attacker vs checker — the genuine engine) and unquantified. **Both follow-ups were subsequently done (HP 1.4.0):** (1) the `p⁴` claim is reframed honestly everywhere it appeared (`plugin.json` / `marketplace.json` / `README` / `start.md` / `ANTI-PATTERNS.md`) — demoted to an idealized model with an explicit shared-model-correlation-floor caveat; (2) `implementer-a` and `implementer-b`, previously byte-identical, are given genuinely different approaches (A spec-first / top-down, B test-first / behavior-driven; same completeness bar, different path) so the weakest independence leg actually decorrelates. A later conversation then applied the **single biggest lever** — **cross-family model assignment** (HP 1.5.0): `implementer-a`/`adversary`/`verifier` on `opus`, `implementer-b`/`critic`/`disproof-agent` on `sonnet`, so builders and checkers span two model families and no longer share one base model's systematic blind spots. (The `model` frontmatter field is family-level, so this pairs Opus 4.8 × Sonnet 5; a specific past version like Opus 4.6 is not selectable, and same-family pairings barely decorrelate. Per-agent *effort* is not frontmatter-configurable — it follows the session `/effort`.)
