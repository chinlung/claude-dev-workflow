...
**Deferred (still open as tracked todos):** openspec pre-check wiring; port audit-review-fix's baseline/exit-sentinel test-regression logic into `review-pr.md`; high-precision `--phase N` (implement or remove) and the missing `disproof-agent` registration; code-audit-rigor `--focus` + Phase 2 confidence field. **Dropped (not pursued):** the canonical-pattern doc (scaffolding for the not-taken "complete everything" path), the skill/audit-review-fix `.cjs`-exposure decision (resolved: existing mechanisms suffice), openspec Phase-6 spec-drift check, debate code-grounding mode. Coverage-declaration schematization for debate (the higher-value L2 stretch) — initially deferred — was subsequently completed in this same consolidation: `debate-output.json` now carries a required, machine-checked `coverage` field, so an uncovered aspect can no longer be silently dropped or left unexplained.
