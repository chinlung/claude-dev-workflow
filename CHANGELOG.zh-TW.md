...
- **scope-ledger 1.0.0** — 每個 worktree 一份工作範圍帳本與四個 fail-open hook，讓 review 驅動的工作不會擴張超出原始請求。帳本（`.claude/scope-ledger.local.md`）記使用者原話、分支、review 輪數與 In scope / Deferred / Log 三段。PreToolUse 在該分支無帳本時把主代理的首次程式碼編輯 deny 一次並要求 `/scope-ledger:scope init`（子代理的編輯放行、不消耗那一次 deny）；PostToolUse 在每次 review 入口 skill 或 review 型子代理之後注入 triage 政策（review 的產出是 input 不是工單——採納、附理由與去處延後、交使用者裁定、或記為噪音，**然後才**動手），只有 allowlist 上的 skill 呼叫計一輪，第 3 輪起收斂告警；Stop 在有未勾項時依狀態 block 一次；SessionStart 在 compaction 或 resume 後把帳本印回。被 git 追蹤的帳本是 repo 控制的內容，四個 hook 一律忽略——Codex 跨 vendor pass 抓到 SessionStart 原本會把一個被 commit 的檔案逐字回放進 context。依量測而非直覺設計：作者全部 session 歷史中 todo 工具零呼叫、一個四天的 session 跑了 23 次安全審查橫跨 17 個 PR、最後使用者在問那些 finding 編號是什麼；另一個有用 task 工具的 session，使用者仍五次問「還剩什麼」，因為 audit 的 backlog 活在任務清單之外。skills.sh 上沒有以 hook 強制的現成方案；最接近的 `piv-fix-review-findings` 是同一政策的 prose 版，本 plugin 採用它的四分類。158 條 fixture 斷言，bash 5 與 macOS bash 3.2 皆綠，九個突變在副本上驗證轉紅。初稿的出貨前審查（五個對抗式驗證子代理、Codex、再加 push 前安全審查）改了七件事才出貨——追蹤中帳本的處理、再補它的 symlink／submodule 繞過、Agent 派發不計輪、以 allowlist 取代關鍵字字根、子代理放行、bump 加鎖、frontmatter 合成——每一件連同理由記在 plugin 自己的 CHANGELOG。設計：`docs/scope-ledger-design-2026-09-23.md`。
